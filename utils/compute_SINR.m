function [SINR sumSigmaQ_ADC]= compute_SINR(H_comm, F_comm, F_sensing, sigmasq_ue, phiADC)
    ...
    %TODO
    sigmaQ_ADC = (sigmasq_ue+comm_signal_pow)*(1-phiADC)*phiADC;
    totalNoise = sigmasq_ue + sigmaQ_ADC/(phiADC^2);
    sumSigmaQ_ADC = sum(sigmaQ_ADC);
    
    for s = 1:size(F_sensing, 1)
        sensing_interference = sensing_interference + abs(sum(sum(conj(H_comm).*F_sensing(s, :, :), 3), 2)).^2;
    end
    SINR= comm_signal_pow./(sum_pow - comm_signal_pow + sensing_interference + totalNoise);
